...
public class JISTIC extends AbstractSig{
    ...
    public void process(ResultData resultData) {
        SNPdata snPdata=new SNPdata(rawMatrix);
        Marker.aberration ab=Marker.aberration.AMP;
        Distribution distribution=new Distribution(snPdata,ab);
        distribution.run();
        List<SignificantRegion> ampRegions= distribution.regions.get(0);
        List<SignificantRegion> delRegions=distribution.regions.get(0);

        /*ab=Marker.aberration.DEL;
        distribution=new Distribution(snPdata,ab);
        distribution.run();*/
...
class Distribution{
    ...
    SNPdata data;
    List<List<SignificantRegion>> regions;
    ...
    void run(){
        ...
        for(Marker.aberration ab : types){
            ...
            if(SignificantRegion.peeloff_gscore_thres[ab.ordinal()] == 0 &&
                ...
                if(Math.abs(SignificantRegion.peeloff_qval_thres - Parameters.qThres) < .0001)
                    SignificantRegion.peeloff_gscore_thres[ab.ordinal()] = Parameters.gThres;
                else {
            ...
            //找显著性区域
            regions.add(SignificantRegion.findRegions(distributions[type.ordinal()],data.markers));
        }
    }
...
class Marker extends GenomeLocation{
    ...
    int GscoreContribution(int type, int sample){
        return (int) Math.abs(Math.round(copy_number[sample]/Parameters.binSize ));
    }
}

class SignificantRegion extends Region{
    ...
    SignificantRegion(Distribution distribution, List<Marker> markers) {
        ...
        while (region_start < markers.size()) {
            ...
            while (region_end < markers.size()
                ...
                region_end++;
            subregions.add(new Region(type, markers.subList(region_start, region_end)));
            region_start = region_end;
        }
        peaks = new Vector<PeakRegion>();
    }

    static List<SignificantRegion> findRegions(Distribution distribution, List<Marker> markers) {
        ...
        for (Iterator<Marker> iter = markers.iterator(); iter.hasNext();)
            peak_qvalue = Math.min(peak_qvalue, iter.next().qScore[type
                    .ordinal()]);
        ...
        for (ListIterator<Marker> iter = markers.listIterator(); iter.hasNext();) {
            ...
            qvalues[iter.previousIndex()] = peel_off_qvalues[iter
                    .previousIndex()] = marker.qScore[type.ordinal()];
        }

        //表示存在峰值
        peak_loop: while (peak_qvalue < Parameters.qThres) {
            ...
            for (int sample = 0; sample < num_samples; sample++)
                if (!peeled_off.get(peak_start).contains(sample)) {
                    ...
                    for (int marker = peak_start; marker < peak_end; marker++)//求Gn值，保存在one_out_peak_gscore中
                        one_out_peak_gscore = Math.max(one_out_peak_gscore,
                                peel_off_Gscores[marker]
                                        - markers.get(marker).GscoreContribution(type.ordinal(), sample));

                    //G值小于等于0时，退出程序
                    if (one_out_peak_gscore <= 0) {
                        Region peak = new Region(type, markers.subList(peak_start, peak_end));
                        System.exit(1);
                    }
                    ...
                    while (peak_start > 0
                            ...
                            .GscoreContribution(type.ordinal(), sample) >= one_out_peak_gscore)//向左扩展区域
                        peak_start--;
                    while (peak_end < markers.size()
                            && peel_off_Gscores[peak_end]
                            - markers.get(peak_end).GscoreContribution(
                            type.ordinal(), sample) >= one_out_peak_gscore)//向右扩展区域
                        peak_end++;
                }
            PeakRegion peak = new PeakRegion(type, markers.subList(peak_start, peak_end), peak_qvalue);
            ...
            if (region_index >= 0 && regions.get(region_index).overlaps(peak))
                ...
                while (start > 0
                        && !(qvalues[start - 1] >= Parameters.qThres))
                    start--;
                while (Double.isNaN(qvalues[start]))
                    start++;
                while (end < markers.size()
                        && !(qvalues[end] >= Parameters.qThres))
                    end++;
            ...
            for (int sample = 0; sample < num_samples; sample++) {
                ...
                if (!peeled_off.get(max_peeloff[1]).contains(sample))
                    remain[sample] = marker.GscoreContribution(type.ordinal(), sample);
            ...
            while (iter.hasNext()) {
                ...
                for (int sample = 0; sample < num_samples; sample++) {
                    int markerCont = 0;
                    if (!peeled_off.get(max_peeloff[1]).contains(sample)) {
                        markerCont = marker.GscoreContribution(type.ordinal(), sample);
                        if (markerCont < remain[sample]) {
                            ...
                            for (int lfmid = max_peeloff[1]; lfmid < end; lfmid++) {
                                //求Gr值，向右
                                markerCont = Math.min(Math.max(markerCont, markers.get(lfmid).GscoreContribution(type.ordinal(), sample)), remain[sample]);
                            }
                        }
                    }
            ...
            for (int sample = 0; sample < num_samples; sample++)
                for (ListIterator<Marker> iter1 = markers
                    ...
                    if (!peeled_off.get(iter1.previousIndex()).contains(sample)) {
                        int peeloff_limits[];
                        ...
                        for (int marker1 = peeloff_limits[0]; marker1 <= peeloff_limits[1]; marker1++)
                            if (peeled_off.get(marker1).add(sample))
                                peel_off_Gscores[marker1] -= markers.get(marker1)
                                        .GscoreContribution(type.ordinal(), sample);
...
class Region extends GenomeLocation{
    Marker.aberration type;
    List<Marker> markers;


    Region(Marker.aberration type, List<Marker> markers){
...
class PeakRegion extends Region {

    double peak_qvalue;
    boolean has_focal = true;

    PeakRegion(Marker.aberration type, List<Marker> markers, double peak_qvalue)
    {
        super(type, markers);
